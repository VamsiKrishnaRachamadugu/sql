...
select e.EMP_ID,e.EMP_NAME,coalesce(d.DESC_NAME,'OTHERS'),coalesce(p.PLACE_DESC,'OTHERS'),count(*) over (partition by d.DESC_NAME) as EMP_DESC_CNT,
count(*) over (partition by p.PLACE_DESC) as EMP_PLACE_CNT from employee as e left join designation as d on e.DESC_ID=d.DESC_ID left join
...
select e.EMP_ID,e.EMP_NAME,coalesce(d.DESC_NAME,'OTHERS'),coalesce(p.PLACE_DESC,'OTHERS'),pr.proj_name from 
employee as e left join designation as d on e.DESC_ID=d.DESC_ID left join
place as p on e.place_id=p.PLACE_ID left join project as pr on e.proj_id=pr.PROJ_ID
where pr.PROJ_NAME like '%Data%' or pr.PROJ_NAME like '%Cloud%';
...
employee as e  join designation as d on e.DESC_ID=d.DESC_ID  join
place as p on e.place_id=p.PLACE_ID  join project as pr on e.proj_id=pr.PROJ_ID) as a where rn_grade=1;
...
select s1.Salesman_Name as sales_manager_name,c.Cust_Name,row_number() over (partition by s1.Salesman_Name order by c.purchased_amount desc) as rn_amnt
...
inner join (
select c.Salesman_Id,max(c.purchased_amount) as max from customer as c group by c.Salesman_Id)as a on
